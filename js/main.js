...
let rateBlack = [];
let rateWhite = [];
let rateYearRangeOne = [];
let rateYearRangeTwo = [];
let rateYearRangeThree = [];
let rateYearRangeFour = [];
let rateDeaths = [];
let rateDeathsByGender = [];
let rateDeathsByRace = [];
let rateDeathsByAge = []
let popData = {};
...
function initMainPage(dataArray) {
    ...
    myBrushVis = new BrushVis('brushDiv', dataArray[1], MyEventHandler);
    myCasualtyVis = new CasualtyVis('totalDiv', dataArray[0]);
    myDotVis = new DotVis('dotDiv', dataArray[0]);
    myPopVis = new PopVis("popDiv", dataArray[0]);
...
let selectedCategoryClimate = $('#climateCategorySelector').val();

function categoryChangeCarbon() {
    selectedCategoryClimate = $('#climateCategorySelector').val();
...
function categoryChange() {
    ...
    myPopVis.wrangleData();// maybe you need to change this slightly depending on the name of your MapVis instance
}
